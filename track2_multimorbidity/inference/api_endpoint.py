...
def load_system_components():
    ...
    telemetry logger, and MLOps monitor at startup.
    """
    ...
    # Core validated schema features
    core_features = list(schema_contract['feature_schema'].keys())

    # Union feature set used during model training
    ...
    telemetry = TelemetryLogger(
        log_dir=LOG_DIR,
        log_filename='api_inference_logs.csv'
    )
...
class PatientVitalsRequest(BaseModel):
    ...
    # Core MIMIC schema features
    glucose_mean: Optional[float] = Field(
        None,
        description="Aggregated glucose level (mg/dL)"
    )

    sbp_mean: Optional[float] = Field(
        None,
        description="Mean systolic blood pressure (mmHg)"
    )

    map_mean: Optional[float] = Field(
        None,
        description="Mean arterial pressure (mmHg)"
    )

    los: Optional[float] = Field(
        None,
        description="ICU length of stay (days)"
    )

    # Pima-aligned metabolic features
    ...
    insulin_score: Optional[float] = Field(
        None,
        description="Insulin resistance proxy"
    )

    genetic_risk_score: Optional[float] = Field(
        None,
        description="Diabetes pedigree function"
    )

    comorbidity_flag: Optional[float] = Field(
        None,
        description="Pregnancies/comorbidity proxy"
    )
...
async def health_check():
    ...
    Reports model status, schema metadata,
    and drift monitor state.
    """
...
async def trigger_manual_retrain(background_tasks: BackgroundTasks):
    ...
    Simulates asynchronous retraining workflow.
    """
...
@app.post(
    "/predict_multimorbidity",
    response_model=InferenceResponse,
    tags=["Inference"]
)
async def predict_crisis(request: PatientVitalsRequest):
    """
    Real-time multimorbidity crisis prediction endpoint.

    Workflow:
    1. Validate core schema features
    2. Pass through additional metabolic features
    3. Align features to trained model feature order
    4. Run inference
    5. Log telemetry
    6. Return standardized Phase 7 payload
    """
    ...
    try:
        ...
        # ---------------------------------------------------------------------
        # STEP 2: Validate only core schema features
        # ---------------------------------------------------------------------
        ...
        if core_input:
            ...
            if not ingestion_result['success']:
                raise HTTPException(
                    status_code=400,
                    detail=f"Core feature validation failed: {ingestion_result.get('errors')}"
                )
        ...
        # ---------------------------------------------------------------------
        # STEP 3: Merge validated core features with passthrough features
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
        # STEP 4: Align features to model training order
        # ---------------------------------------------------------------------

        if not hasattr(model, 'feature_names_in_'):
            raise RuntimeError(
                "Model does not expose feature_names_in_. "
                "Ensure model was trained using sklearn-compatible API."
            )
        ...
        if proba > 0.75:
            ...
            if processed_features.get('glucose_mean', 0) > 150:
                crisis_type = "isolated_glucose"
            else:
                crisis_type = "isolated_bp"

        # ---------------------------------------------------------------------
        # STEP 7: Temporal instability proxy
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
        # STEP 8: Approximate SHAP-style driver ranking
        # ---------------------------------------------------------------------

        shap_drivers = sorted(
            processed_features.items(),
            key=lambda x: abs(
                x[1] - schema_contract['feature_schema'].get(
                    x[0],
                    {}
                ).get('q50', 0)
            ) if x[0] in schema_contract['feature_schema']
            else abs(x[1]),
            reverse=True
        )[:3]
        ...
        # ---------------------------------------------------------------------
        # STEP 9: Approximate confidence interval
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
        # STEP 11: Construct response payload
        # ---------------------------------------------------------------------
...
async def startup_event():
    """
    Start background MLOps drift monitoring loop
    when the FastAPI application boots.
    """

    asyncio.create_task(
        mlops_monitor.monitor_loop(
